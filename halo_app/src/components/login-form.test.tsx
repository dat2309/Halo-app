...
describe('LoginForm Form ', () => {
  ...
  it('shows name field only on register', async () => {
    setup(<LoginForm variant="login" />);
    expect(screen.queryByTestId('name')).not.toBeOnTheScreen();
    cleanup();
    setup(<LoginForm variant="register" />);
    expect(await screen.findByTestId('name')).toBeOnTheScreen();
  });
});
